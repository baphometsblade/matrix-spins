...
// ═══════════════════════════════════════════════════════
        ...
        function findClusters(grid, game) {
            ...
            for (let c = 0; c < cols; c++) {
                for (let r = 0; r < rows; r++) {
                    ...
                    if (!symbol) continue;
                    // Skip wilds as cluster seeds — they join adjacent clusters instead
                    if (isWild(symbol, game)) continue;

                    // BFS to find cluster (index pointer avoids O(n) shift)
                    const cluster = [];
                    const queue = [[c, r]];
                    let qi = 0;
                    visited[c][r] = true;
                    ...
                    while (qi < queue.length) {
                        ...
                        for (const [nc, nr] of neighbors) {
                            ...
                            // Wilds substitute for any symbol; regular symbols must match
        ...
        function applyBonusMultiplier(baseWin, game) {
            ...
            if (freeSpinsActive && game.bonusType === 'random_multiplier') {
                ...
                multiplier = baseMultChoices[Math.floor(getRandomNumber() * baseMultChoices.length)];
            ...
            if (freeSpinsActive && game.bonusType === 'zeus_multiplier') {
                ...
                if (getRandomNumber() < 0.20) {
                    multiplier = 2 + Math.floor(getRandomNumber() * 3); // 2, 3, or 4
                    bonusText = '⚡ ×' + multiplier + ' Zeus!';
                }
            }

            if (freeSpinsActive && (game.bonusType === 'tumble' || game.bonusType === 'avalanche' || game.bonusType === 'cascading')) {
            ...
            } else if (!freeSpinsActive && (game.bonusType === 'tumble' || game.bonusType === 'avalanche' || game.bonusType === 'cascading')) {
                // Base game: use the running tumble chain multiplier if > 1
                // ROUND 33: Cap at 5x (was completely unbounded — console exploit: window._tumbleChain = 99999)
                if (typeof window._tumbleChain !== 'undefined' && window._tumbleChain > 1) {
                    multiplier = Math.min(window._tumbleChain, 5);
        ...
        function getMoneyValue(symbol, game) {
            ...
            // Realistic money values: fractions of bet (real Hold & Win values)
            const values = [0.5, 1, 1.5, 2, 3, 5, 10, 25];
        ...
        function checkWin(symbols, game = currentGame) {
            ...
            if (winType === 'cluster' && grid) {
                ...
                for (const cluster of clusters) {
                    ...
                    if (window.HouseEdge) {
                        ...
                        // Fallback: realistic cluster pay fractions (aligned to ~96% RTP)
                        if (size >= 15) payMultiplier = 5.0;
                        else if (size >= 12) payMultiplier = 2.0;
                        else if (size >= 10) payMultiplier = 0.8;
                        else if (size >= 8) payMultiplier = 0.35;
                        else if (size >= 6) payMultiplier = 0.15;
                        else if (size >= 5) payMultiplier = 0.08;
                    }
            ...
            // ═══ PAYLINE WIN DETECTION ═══
                ...
                for (const win of paylineWins) {
                    ...
                    // Wild bonus: up to 2x multiplier on wild-assisted wins
                    const lineSymbols = win.cells.map(([c, r]) => grid[c][r]);
                    const lineWilds = lineSymbols.filter(s => isWild(s, game)).length;
                    if (lineWilds > 0) payMultiplier *= Math.min(2, 1 + lineWilds * 0.25);
                ...
                if (game.bonusType === 'chamber_spins') {
                    ...
                    if (typeof triggerPrizeWheel === 'function') {
                        ...
                        if (typeof onChallengeEvent === 'function') onChallengeEvent('bonus', { gameId: game ? game.id : null });
                           ...
                           game.bonusType === 'cascading' || game.bonusType === 'expanding_wilds' ||
                           game.bonusType === 'random_jackpot' || game.bonusType === 'symbol_collect' ||
                           ...
                           game.bonusType === 'both_ways' || game.bonusType === 'colossal' ||
                           game.bonusType === 'wild_reels') {
                    // These types use standard free spins — the per-spin mechanic fires
                    // inside displayServerWinResult hooks (ui-slot.js) based on bonusType.
                    // both_ways: reverse-eval wins added each spin
                    // colossal: 25% chance 2×2 block lands each spin
                    // wild_reels: random reel becomes all wilds during free spins
                    playSound('freespin');
            ...
            if (scatterCount >= scatterThreshold && freeSpinsActive && game.freeSpinsRetrigger && freeSpinsRemaining < MAX_FREE_SPINS) {
                ...
                if (game.bonusType === 'chamber_spins') {
                    ...
                    if (capped > 0) {
                        ...
                        if (typeof freeSpinsTotalCount !== 'undefined') freeSpinsTotalCount += capped; // Track retrigger for "X of Y"
