(function() {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      messageInput.value = '';
      var _charCount = document.getElementById('chat-char-count');
      if (_charCount) _charCount.textContent = `0/${MAX_MESSAGE_LENGTH}`;
  ...
  function hide() {
    if (!chatPanel) return;
    isVisible = false;
    stopPolling(); // Stop background requests when chat is hidden
  ...
  // Pause polling when tab is hidden, resume when visible (if chat is open)
