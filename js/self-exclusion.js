    ...
    function showSelfExclusionModal() {
        ...
        for (var i = 0; i < btns.length; i++) {
            btns[i].addEventListener('click', function() {
                ...
                var _seText = document.getElementById('se-confirm-text');
                var _seArea = document.getElementById('se-confirm-area');
                var _seOpts = document.getElementById('se-options');
                if (_seText) _seText.textContent = 'You are about to exclude yourself for ' + labels[selectedType] + '. During this period you will not be able to place any bets or play any games.';
                if (_seArea) _seArea.style.display = 'block';
                if (_seOpts) _seOpts.style.display = 'none';
            });
        }
