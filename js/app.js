...
// ═══════════════════════════════════════════════════════
        ...
        function loadState() {
            // ROUND 40: Only load localStorage balance for guest/local users.
            // Server-authenticated users get balance from /api/balance on login.
            var token = typeof localStorage !== 'undefined' ? localStorage.getItem('casinoAuthToken') : null;
            var isServerAuth = token && !token.startsWith('local_');
            if (!isServerAuth) {
        ...
        function saveBalance() {
            ...
            if (token && !token.startsWith('local_')) {
                // Server-authenticated: do NOT persist client balance to localStorage.
                // Balance is managed server-side; client display refreshes from /api/balance.
                return;
            }
        ...
        function onPostAuthInit() {
            ...
            if (typeof KeyboardShortcuts !== 'undefined' && KeyboardShortcuts.init && !suppressBonus) {
            ...
            switch (e.code) {
                ...
                case 'Escape':
                    ...
                    if (paytable && paytable.classList.contains('active')) {
                        togglePaytable();
                    } else if ((document.getElementById('settingsModal') || {}).classList && document.getElementById('settingsModal').classList.contains('active')) {
                        closeSettingsModal();
                    } else if ((document.getElementById('statsModal') || {}).classList && document.getElementById('statsModal').classList.contains('active')) {
                        closeStatsModal();
                    } else if (slotOpen) {
                        closeSlot();
                    }
...
// === FALLBACK: Ensure games render even if rAF/setTimeout fails ===
(function _ensureGamesRender() {
    function _tryRender() {
        if (typeof games === 'undefined' || !Array.isArray(games) || games.length === 0) return;
        var hot = document.getElementById('hotGames');
        var all = document.getElementById('allGames');
        if (!hot || !all) return;
        // Check if still showing skeletons
        if (hot.querySelector('.game-card-skeleton') || all.querySelector('.game-card-skeleton') || hot.children.length === 0) {
            try {
                var hotGames = games.filter(function(g) { return g.hot; });
                hot.innerHTML = hotGames.map(function(g) { return createGameCard(g); }).join('');
                if (typeof renderFilteredGames === 'function') renderFilteredGames();
                if (typeof _injectLobbySearch === 'function') _injectLobbySearch();
                if (typeof renderRecentlyPlayed === 'function') renderRecentlyPlayed();
                if (typeof renderYouMightLike === 'function') renderYouMightLike();
                console.warn('[Fallback] Games rendered: ' + hotGames.length + ' hot, ' + games.length + ' total');
            } catch(e) { console.warn('[Fallback] render error:', e); }
        }
    }
    // Try at multiple intervals to be safe
    setTimeout(_tryRender, 500);
    setTimeout(_tryRender, 1500);
    setTimeout(_tryRender, 3000);
})();
