...
// ═══════════════════════════════════════════════════════
        ...
        // Safe localStorage wrapper — prevents QuotaExceededError crashes
        function _safeSetItem(key, val) {
            try { localStorage.setItem(key, val); } catch (_) { console.warn('[Auth] localStorage quota exceeded for key:', key); }
        }
        ...
        function applyAuthSession(token, user) {
            ...
            authToken = token;
            _safeSetItem(STORAGE_KEY_TOKEN, token);
            currentUser = user ? {
                id: user.id,
            ...
            _safeSetItem(STORAGE_KEY_USER, JSON.stringify(currentUser));
            ...
            try {
                ...
                let xpData = {};
                try { xpData = JSON.parse(localStorage.getItem(xpKey) || '{}'); } catch (_) {}
        ...
        async function loginWithLocalFallback(username, password) {
            let users = {};
            try { users = JSON.parse(localStorage.getItem(STORAGE_KEY_USERS) || '{}'); } catch (_) {}
            ...
            if (isLegacyMatch) {
                ...
                users[key] = user;
                _safeSetItem(STORAGE_KEY_USERS, JSON.stringify(users));
            }
