...
// ═══════════════════════════════════════════════════════
        ...
        function generateSpinResult(game, isFreeSpins) {
            // ROUND 31: forcedSpinQueue disabled for authenticated users.
            // Was exploitable via console: players could queue guaranteed jackpot outcomes.
            // Only demo mode (no auth token, no real money) can use forced spins.
            if (forcedSpinQueue.length > 0 && !(typeof isServerAuthToken === 'function' && isServerAuthToken())) {
        ...
        function queueForcedSpin(symbolsInput) {
            // ROUND 31: Block for authenticated (real-money) users
            if (typeof isServerAuthToken === 'function' && isServerAuthToken()) {
                console.warn('[Security] Forced spins disabled for authenticated users');
                return null;
            }
        ...
        function queueForcedOutcome(type, preferredSymbol) {
            // ROUND 31: Block for authenticated (real-money) users
            if (typeof isServerAuthToken === 'function' && isServerAuthToken()) {
                console.warn('[Security] Forced outcomes disabled for authenticated users');
                return null;
            }
