...
(function() {
    ...
    // HTML escape helper to prevent XSS in user-sourced strings
    ...
    async function refreshPlayers() {
        try {
            ...
            whalers.forEach(function(p) {
                ...
                html += '<tr>' +
                    '<td>' + _escHtml(p.display_name || p.username) + '</td>' +
    ...
    function hide() {
        ...
        state.visible = false;
        if (state.refreshTimer) clearInterval(state.refreshTimer);
        if (_refreshDisplayTimer) { clearInterval(_refreshDisplayTimer); _refreshDisplayTimer = null; }
    }
    ...
    var _refreshDisplayTimer = setInterval(updateLastRefreshDisplay, 5000);
