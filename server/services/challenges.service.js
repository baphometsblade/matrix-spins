...
function _pickRandom(arr) {
    return arr[Math.floor((crypto.randomBytes(4).readUInt32BE(0) / 0x100000000) * arr.length)];
}
...
async function getDailyChallenges(userId) {
    ...
    var existing = await db.all(
        "SELECT id, user_id, date, challenge_type, target, progress, reward_credits, reward_gems, completed, difficulty, description FROM daily_challenges WHERE user_id = ? AND date = date('now') ORDER BY id",
        [userId]
    );
    ...
    for (var i = 0; i < picks.length; i++) {
        ...
        var row = await db.get(
            "SELECT id, user_id, date, challenge_type, target, progress, reward_credits, reward_gems, completed, difficulty, description FROM daily_challenges WHERE user_id = ? AND date = date('now') AND challenge_type = ? AND difficulty = ? ORDER BY id DESC LIMIT 1",
...
async function updateProgress(userId, challengeType, amount, extra) {
    ...
    var challenges = await db.all(
        "SELECT id, progress, target, completed, reward_credits, reward_gems FROM daily_challenges WHERE user_id = ? AND date = date('now') AND challenge_type = ? AND completed = 0",
...
async function _updateStreak(userId) {
    ...
    var row = await db.get(
        'SELECT current_streak, best_streak, last_completed_date FROM challenge_streaks WHERE user_id = ?',
        [userId]
    );
...
async function skipChallenge(userId, challengeId) {
    ...
    var challenge = await db.get(
        "SELECT id, completed, reward_credits, reward_gems, target FROM daily_challenges WHERE id = ? AND user_id = ? AND date = date('now')",
        [challengeId, userId]
    );
