...
.Groups[2].Value
        # Add our pages if not already there
        $toAdd = @()
        if ($existing2 -notmatch "launch\.html") { $toAdd += "'launch.html'" }
        if ($existing2 -notmatch "affiliates\.html") { $toAdd += "'affiliates.html'" }
        if ($existing2 -notmatch "referral\.html") { $toAdd += "'referral.html'" }
        if ($existing2 -notmatch "wallet\.html") { $toAdd += "'wallet.html'" }
        if ($existing2 -notmatch "login\.html") { $toAdd += "'login.html'" }
        if ($existing2 -notmatch "signup\.html") { $toAdd += "'signup.html'" }
        $addStr = if ($toAdd) { ", " + ($toAdd -join ", ") } else { "" }
        #!/usr/bin/env node
...
.Groups[1].Value + $existing2.TrimEnd() + $addStr + #!/usr/bin/env node
